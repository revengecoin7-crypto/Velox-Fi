...
- **Skill games (4 live)**: Rocket Miner (`/games/rocket-miner`), Crypto Snake (`/games/crypto-snake`), Battle Tetris (`/games/battle-tetris`), Wolf Run (`/games/wolf-run`). All earn up to 120 WOLF/session, record to `veloxfi_game_sessions`, and have a CLAIM WOLF button. Game sessions drive GAMES ACTIVITY and DAILY MISSIONS on the profile. CSS prefixes: `rmp-`, `cs-`, `bt-`, `wr-`.
- **Wolf Run game** (`/games/wolf-run`): Canvas-based endless side-scrolling runner. Wolf fixed at x=90, player jumps with SPACE/↑/tap to avoid rocks and barriers. Gold ⚔ coins auto-collected = 1 WOLF each. Scrolling cyberpunk city skyline (2 parallax building layers + stars + neon ground). 2-minute countdown, 3 lives, invincibility frames after hit. Mobile JUMP button shown on touch devices. Claim endpoint: `POST /api/veloxfi/game/wolf-run/earn` (cap 120 WOLF, records to `veloxfi_game_sessions`).
